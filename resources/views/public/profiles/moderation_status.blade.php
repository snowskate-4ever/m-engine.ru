@php
    $o = $overlay ?? 'pending';
@endphp
<x-layouts.public-minimal :title="__('ui.public_profile.moderation_'.$o.'_title')">
    <main class="mx-auto flex min-h-svh max-w-lg flex-col items-center justify-center px-6 py-16 text-center">
        ...
        <h1 class="mt-2 text-xl font-semibold tracking-tight">{{ __('ui.public_profile.moderation_'.$o.'_heading') }}</h1>
        <p class="mt-4 text-sm leading-relaxed text-zinc-600 dark:text-zinc-300">
            {{ __('ui.public_profile.moderation_'.$o.'_body') }}
        </p>
