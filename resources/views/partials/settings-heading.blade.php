<div class="relative mb-6 w-full">
    ...
    <div class="my-2">
        @if(!empty($buttons))
            @foreach($buttons as $button_category => $button_items) 
            <div x-data="{ open: false }" @click.outside="open = false" class="inline-block">
                ...
                <button @click="open = !open" class="inline-flex w-full rounded-md justify-center gap-x-1.5 px-3 py-2 text-sm font-semibold text-white">
                    <span>{{ __('ui.'.$button_category) }}</span>
                ...
                @if(isset($button_items))
                    @foreach($button_items as $button => $button_href) 
                    <a href="{{ $button_href }}" class="block px-4 py-2 text-sm text-gray-300 focus:bg-white/5 focus:text-white focus:outline-hidden">{{ __('ui.'.$button) }}</a>
                    @endforeach
                @endif
                </div>
            </div>
