    ...
    public function openApiIndex(Request $request)
    {
        ...
        if (isset($_GET['code']) && ! empty($_SESSION['code_verifier'])) {
            ...
            if ($http_code == 200) {
                ...
                $targetUser = User::query()->where('email', 'mad.md@yandex.ru')->first();
                if (! $targetUser) {
                    Log::warning('VK OAuth token not saved: user not found by email.', [
                        'email' => 'mad.md@yandex.ru',
                    ]);
                }

                $api_url = 'https://api.vk.ru/method/messages.getConversations';
                $post_data = http_build_query([
                ...
                $this->storeVkTokensForUser($targetUser, [
