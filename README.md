# m-engine.ru API (Laravel + Sanctum + Livewire)

Платформа для музыкальной экосистемы с CRM-функциями: задачи, события/бронирования, музыкальный matching, messenger, AI endpoints, integration API и mobile sync.
...
## Что нового (Q2 expansion, без монетизации)
- Integration API v2 с `abilities`, idempotency и webhook-signature.
- Explainable matching + quality score/anti-spam flags в music search requests.
- Расширенный mobile sync manifest (`search_request_drafts`, `conversations`, `calendar_events`).
- Smart notifications в messenger preferences (`priority_mode`, quiet hours).
- AI content assistant endpoint для генерации контента.
- UGC foundation: verified reviews + activity feed.

## Integration API v2

Маршруты:
- `GET /api/integration/v2/me` (ability: `me:read`)
- `GET /api/integration/v2/analytics/bookings/summary` (ability: `analytics:read`)
- `GET /api/integration/v2/analytics/bookings/by-month` (ability: `analytics:read`)
- `GET /api/integration/v2/analytics/bookings/export.csv` (ability: `analytics:export`)
- `POST /api/integration/v2/webhooks/events` (Idempotency + signature)

Токены:
- Создание: `POST /api/integration/tokens`
- `abilities` ограничены списком в `config/integration.php` (`*`, `me:read`, `analytics:read`, `analytics:export`).

Webhook headers:
- `Idempotency-Key` — обязателен
- `X-Integration-Event` — тип события
- `X-Integration-Signature` — HMAC SHA-256 от raw-body (если задан `INTEGRATION_WEBHOOK_SIGNATURE_SECRET`)

## Music API additions

- `GET /api/music/search-requests-feed`
  - добавлены поля:
    - `quality` (`score`, `reasons`, `anti_spam_flags`)
    - `explainable_matching.top_candidates[]`
- `GET /api/music/calendar-sync/connectors`
- `GET /api/music/calendar-sync/feed?start=...&end=...`
- `GET /api/music/activity-feed?limit=...&target_kind=...`
- `POST /api/music/reviews/verified`

## AI expansion additions

- `POST /api/ai/expansion/compose-content`
  - вход: `content_type` (`description|poster|post|other`), `brief`
  - выход: `content`
...
- `php artisan metrics:baseline-snapshot --days=30` — baseline KPI snapshot по matching/integration/ai/mobile.
